...
from tensorflow.contrib.rnn import LSTMCell, RNNCell, DropoutWrapper, MultiRNNCell
from rnn import stack_bidirectional_dynamic_rnn, CellInitializer, GRUCell, DropoutGRUCell, PLSTM
from rnn import get_state_size
...
class BaseModel(object):
    ...
    def __init__(self, hparams, mode, iterator, target_vocab_table, reverse_target_vocab_table=None, scope=None, single_cell_fn=None):
        ...
        print('bink_0514', self.cnn_input)
        ...
        print('bink_0514',self.batch_size)
        #注意：source_sequence_length是不定长的，并不是指300,是pad之前的；而注意batch.size是指1
    ...
    def build_graph(self, hparams, scope=None):
        ...
        with tf.variable_scope(scope or "dynamic_seq2seq", dtype=dtype):
            # Encoder
            encoder_outputs, encoder_state,new_encoder_input_length = self._build_encoder(hparams)

            ## Decoder
            logits, sample_id, final_context_state = self._build_decoder(encoder_outputs, encoder_state, new_encoder_input_length,hparams)
    ...
    def _build_decoder(self, encoder_outputs, encoder_state, new_encoder_input_length, hparams):
        ...
        if hparams.tgt_max_len_infer:
            ...
            max_encoder_length = tf.reduce_max(new_encoder_input_length)
            maximum_iterations = tf.to_int32(tf.round(tf.to_float(max_encoder_length) * decoding_length_factor))

        ## Decoder.
        with tf.variable_scope("decoder") as decoder_scope:

            cell, decoder_initial_state = self._build_decoder_cell(hparams, encoder_outputs, encoder_state, new_encoder_input_length)
...
class CellWrapper(RNNCell):
    """
    Wrapper around LayerNormBasicLSTMCell, LSTMCell and MultiRNNCell, to keep
...
class Model(BaseModel):
    ...
    def _build_encoder(self, hparams):
        ...
        def get_cell(input_size=None, reuse=False):
            if hparams.cell_type.lower() == 'lstm':
                cell = CellWrapper(LSTMCell(hparams.cell_size, reuse=reuse))
            elif hparams.cell_type.lower() == 'plstm':
                cell = PLSTM(hparams.cell_size, reuse=reuse, fact_size=hparams.lstm_fact_size,
                             proj_size=hparams.lstm_proj_size)
            elif hparams.cell_type.lower() == 'dropoutgru':
                cell = DropoutGRUCell(hparams.cell_size, reuse=reuse, layer_norm=hparams.layer_norm,
                                      input_size=input_size, input_keep_prob=hparams.rnn_input_keep_prob,
                                      state_keep_prob=hparams.rnn_state_keep_prob)
            else:
                cell = GRUCell(hparams.cell_size, reuse=reuse, layer_norm=hparams.layer_norm)

            if hparams.use_dropout and hparams.cell_type.lower() != 'dropoutgru':
                    cell = DropoutWrapper(cell, variational_recurrent=hparams.pervasive_dropout,
        ...
        output = self.cnn.output    #encoder的输入  [300,4096]
        output = tf.nn.tanh(output) #经过两层tanh
        output = tf.nn.tanh(output)

        output = tf.expand_dims(output,0)   #[1,300,4096]

        cell_output_size, cell_state_size = get_state_size(hparams.cell_type, hparams.cell_size)
        encoder_inputs_ = output
        encoder_input_length_ = iterator.source_sequence_length
        if hparams.pos_embedding_size:  #待改进 position embedding
            pos_embedding_shape = [hparams.src_max_len, hparams.pos_embedding_size]
            pos_embeddings = get_variable('pos_embedding_encoder', shape=pos_embedding_shape)

            pos_inputs_ = tf.range(300, dtype=tf.int32)#输出 [0 1 .... 300]   300=time_steps
            pos_inputs_ = tf.nn.embedding_lookup(pos_embeddings, pos_inputs_)
            print("pos_inputs_", pos_inputs_)  # shape=(300, 250)
            pos_inputs_ = tf.tile(tf.expand_dims(pos_inputs_, axis=0), [self.batch_size, 1, 1])
            print("pos_inputs_",pos_inputs_)    #shape=(1, 300, 250)

            encoder_inputs_ = tf.concat([encoder_inputs_, pos_inputs_], axis=2) #pos_inputs_: [1,300,250]
            print("encoder_inputs_",encoder_inputs_)    #shape=(1, 300, 4346)

        if hparams.conv_filters:
            encoder_inputs_ = tf.expand_dims(encoder_inputs_, axis=3)   #[batch_size,300,4096,1] #最后一位是chnanel
            for k, out_channels in enumerate(hparams.conv_filters, 1):  #k=1,out_channels=16;k=2,out_channels=16    #执行两次
                in_channels = encoder_inputs_.get_shape()[-1].value     # 维度为1
                filter_height, filter_width = hparams.conv_size     # [3,3]
                strides = hparams.conv_strides or [1, 1]    # [2,2]
                strides = [1] + strides + [1]   #  [1,2,2,1]

                filter_ = get_variable('filter_{}'.format(k),
                                       [filter_height, filter_width, in_channels, out_channels])
                encoder_inputs_ = tf.nn.conv2d(encoder_inputs_, filter_, strides, padding='SAME')

                if hparams.conv_activation is not None and hparams.conv_activation.lower() == 'relu':   #默认为None
                    encoder_inputs_ = tf.nn.relu(encoder_inputs_)

                print("encoder_inputs_",encoder_inputs_)    #第一次shape=(1, 150, 2173, 16)；第二次：shape=(1, 75, 1087, 16)

                encoder_input_length_ = tf.to_int32(tf.ceil(encoder_input_length_ / strides[1]))
                #tf.ceil(x) 返回不下于x的最小整数
                print("encoder_input_length_",encoder_input_length_)    #shape=(1,)

            feature_size = encoder_inputs_.shape[2].value
            channels = encoder_inputs_.shape[3].value
            time_steps = tf.shape(encoder_inputs_)[1]

            encoder_inputs_ = tf.reshape(encoder_inputs_, [self.batch_size, time_steps, feature_size * channels])
            conv_outputs_ = encoder_inputs_
            print("conv_outputs_",conv_outputs_)    #shape=(1, 75, 17392)
        ...
        with tf.variable_scope("encoder") as scope:
            ...
            # num_bi_layers = hparams.layers
            # num_bi_residual_layers = hparams.layers
            utils.print_out("  num_bi_layers = %d, num_bi_residual_layers=%d" % (num_bi_layers, num_bi_residual_layers))

            encoder_outputs0, _ = (self._build_bidirectional_rnn(inputs=tf.transpose(conv_outputs_,perm=[1,0,2]),
            ...
            encoder_outputs1, bi_encoder_state = (
                self._build_bidirectional_rnn(inputs=encoder_outputs0,
                                              sequence_length=encoder_input_length_,
                                              dtype=dtype,
                                              hparams=hparams,
                                              num_bi_layers=num_bi_layers,
                                              num_bi_residual_layers=num_bi_residual_layers,
                                              scope_name='2nd_lstm'))
            print('sssssssssssssssss',encoder_outputs1)
            encoder_outputs, bi_encoder_state = (
                self._build_bidirectional_rnn(inputs=encoder_outputs1,
                                              sequence_length=encoder_input_length_,
                                              dtype=dtype,
                                              hparams=hparams,
                                              num_bi_layers=num_bi_layers,
                                              num_bi_residual_layers=num_bi_residual_layers,
                                              scope_name='3rd_lstm'))
            print('sssssssssssssssss', encoder_outputs)
            if num_bi_layers == 1:
            ...
            print("asdas",encoder_outputs, encoder_state, encoder_input_length_)
        ...
        #     batch_norm=None, inter_layer_keep_prob=None,
        #     pervasive_dropout=True, training=True
        # )
        ...
        #               for j in range(hparams.layers)],
        #     cells_bw=[get_cell(input_size if j == 0 else 2 * cell_output_size, reuse=reuse)
        #               for j in range(hparams.layers)],
        #     initial_states_fw=[None] * hparams.layers,
        #     initial_states_bw=[None] * hparams.layers,
        #     time_pooling=False, pooling_avg=False,
        #     **parameters)
        #
        # initializer = None
        # with tf.variable_scope(tf.get_variable_scope(), initializer=initializer):
        #     try:
        #         encoder_outputs_, encoder_states_f, encoder_states_b = rnn(reuse=False)
        #     except ValueError:
        #         encoder_outputs_, encoder_states_f, encoder_states_b = rnn(reuse=True)
        #
        # print("encoder_outputs_",encoder_outputs_)  #shape=(1, 75, 512)
        # print("encoder_states_",encoder_states_f,encoder_states_b)    #shape=(1, 512)
        #
        #
        # if hparams.final_state == 'concat_last':  # concats last states of all backward layers (full LSTM states)
        #     encoder_state_f = tf.concat(encoder_states_f, axis=1)
        #     encoder_state_b = tf.concat(encoder_states_b, axis=1)
        #
        # if hparams.bidir and hparams.bidir_projection:
        #     encoder_outputs_ = dense(encoder_outputs_, cell_output_size, use_bias=False, name='bidir_projection')
        #
        # encoder_state_ = tf.transpose(encoder_state_f,perm=[1,0])
        #
        #
        # encoder_outputs_ = tf.transpose(encoder_outputs_, perm=[1, 0, 2])
        #
        # print("encoder_outputs_",encoder_outputs_)  #shape=(1, 75, 256)
        ...
        # print("binkkencoder_state000", encoder_states)  #shape=(1, 1536)
        # encoder_state = tf.concat(encoder_states, 1)

        # print("binkk",encoder_outputs)  #[<tf.Tensor 'dynamic_seq2seq/bidir_projection/Tensordot:0' shape=(1, 75, 256) dtype=float32>]
        # print("binkkencoder_state",encoder_state)   #Tensor("dynamic_seq2seq/concat_2:0", shape=(1, 1536), dtype=float32, device=/device:GPU:0)

        # return encoder_outputs_, encoder_state_, encoder_input_length_
        return encoder_outputs, encoder_state, encoder_input_length_

    def _build_bidirectional_rnn(self, inputs, sequence_length, dtype, hparams, num_bi_layers, num_bi_residual_layers,scope_name=None):
