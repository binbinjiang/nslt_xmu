...
def read_video(src, source_reverse):
    #sr 是 b前缀的字符串 需要用decode()解码

    print("src:",src)
    # print("sourcessssssssss",source_reverse)
    images = sorted([f for f in listdir(src) if isfile(join(src, f))])
    # print(images)
    video = np.zeros((len(images), 227, 227, 3)).astype(np.float32)
    # video = np.zeros((len(images), 227, 227, 3))
    #print('~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~dddddddddddd~~~~~~~~~~~~~~~~~')

    # Cihan_CR: Harcoded Path, Need to Change This
    mean_image = np.load('../Mean/FulFrame_Mean_Image_227x227.npy').astype(np.float32)[..., ::-1]
    # mean_image = np.load('../../Mean/FulFrame_Mean_Image_227x227.npy')[..., ::-1]
    # print(type(mean_image))
    # for each image
    for i in range(0, len(images)):
        # video[i, :, :, :] = cv2.imread(src + images[i]).astype(np.float32) - mean_image
        # print(cv2.imread(src + images[i]))
        # print(type(cv2.imread(src + images[i])))
        # print('dasdaddaddasd',src+b'/'+images[i])
        video[i, :, :, :] = cv2.imread((src + b'/'+ images[i]).decode())- mean_image
