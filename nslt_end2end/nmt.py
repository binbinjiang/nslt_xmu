...
def add_arguments(parser):
    ...
    parser.add_argument("--encoder_type",               type=str,       default="bi",  help="""\ uni | bi | gnmt. For bi, we build num_layers/2 bi-directional layers.For gnmt, we build 1 bi-directional layer, and (num_layers - 1) unidirectional layers. """)
    ...
    parser.add_argument("--cuda_visible_devices", type=str, default=0,
                        help="Set the id number for os.environ['CUDA_VISIBLE_DEVICES'].e.g.(only one): 3 ;(one more): 1,2,3 ")

    #encoder
    parser.add_argument("--pos_embedding_size", type=int, default=250, help="Position embedding size.")
    parser.add_argument("--layers", type=int, default=3, help="layers of BLSTM")
    parser.add_argument("--conv_filters", type=list, default=[8, 8], help="")
    parser.add_argument("--conv_size", type=list, default=[3, 3], help="")
    parser.add_argument("--conv_strides", type=list, default=[2, 2], help="")
    parser.add_argument("--conv_activation", type=str, default=None, help="")
    # parser.add_argument("--input_layers", type=int, default=[256, 128], help="")
    parser.add_argument("--bidir_projection", type="bool", default=True, help="")
    parser.add_argument("--bidir", type="bool", default=True, help="")
    parser.add_argument("--final_state", type=str, default="concat_last", help="")
    parser.add_argument("--input_layer_dropout", type=float, default=0.4, help="")
    parser.add_argument("--binary", type="bool", default=True, help="")
    parser.add_argument("--cell_type", type=str, default="lstm", help="")
    parser.add_argument("--cell_size", type=int, default=256, help="")
    parser.add_argument("--attn_size", type=int, default=256, help="")
    parser.add_argument("--use_dropout", type="bool", default=True, help="")
    parser.add_argument("--pervasive_dropout", type="bool", default=True, help="")
    parser.add_argument("--attn_dropout", type=float, default=0.4, help="")
    parser.add_argument("--rnn_input_dropout", type=float, default=0.4, help="")
    parser.add_argument("--initial_state_dropout", type=float, default=0.4, help="")
    parser.add_argument("--train_initial_states", type="bool", default=False, help="")
    parser.add_argument("--time_pooling", type="bool", default=False, help="")
...
def create_hparams(flags):
    ...
    return tf.contrib.training.HParams(
        ...
        # input_layers = flags.input_layers,
        bidir_projection = flags.bidir_projection,
        bidir = flags.bidir,
        final_state = flags.final_state,
        input_layer_dropout = flags.input_layer_dropout,
        binary = flags.binary,
        cell_type = flags.cell_type,
        cell_size = flags.cell_size,
        attn_size = flags.attn_size,
        use_dropout = flags.use_dropout,
        pervasive_dropout = flags.pervasive_dropout,
        attn_dropout = flags.attn_dropout,
        rnn_input_dropout = flags.rnn_input_dropout,
        initial_state_dropout = flags.initial_state_dropout,
        train_initial_states = flags.train_initial_states,
        time_pooling = flags.time_pooling,
...
if __name__ == "__main__":
    ...
    nmt_parser = argparse.ArgumentParser()  #创建一个解析对象
    add_arguments(nmt_parser)   # 向该对象中添加你要关注的命令行参数和选项
    FLAGS, unparsed = nmt_parser.parse_known_args() #进行解析
    print(FLAGS.dev_prefix)
    print(FLAGS.vocab_prefix)
